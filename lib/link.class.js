/* This class represents a telehash link, containing one or more pipes (paths)
 * https://github.com/telehash/telehash.org/blob/master/v3/link.md
 * @author Ryan Bennett, refactoring from mesh.js by Jeremie Miller
 */
...
TLink.prototype.sync = function TLink_sync()
{
  ...
  var pipes = link.pipes;

  for (var i in pipes){
    //ES15: let pipe = pipes[i];
...
TLink.prototype.status = function TLink_addStatusListener(cb){
  ...
  if(this.down != 'init'){
    function make_status_listener(link, cb){
      return function status_listener(){
        link.emit('status', link.down, link);
      };
    }
    process.nextTick(make_status_listener(this, cb));
  }
...
TLink.prototype.addPipe = function TLink_addPipe(pipe, see)
{
  ...
  {
    log.debug(this.hashname,'adding new pipe',pipe.path);
...
TLink.prototype._sortPipes = function TLink__sortPipes(){
  this.pipes = this.pipes.sort(function sortPipes(a,b){
    ...
    if(seenA >= this.syncedAt && seenB >= this.syncedAt)
      return seenA - seenB;
    // if either is in sync, prefer them
    else if(seenA >= this.syncedAt)
      return -1;
    else if(seenB >= this.syncedAt)
      return 1;
...
TLink.prototype.router = function(router)
{
 ...
 TLink.prototype.setInfo = function TLink_setInfo(){

   // update/set json info
   this.json = this._mesh.jsonize(this.hashname, this.args);
   this.csid = hashname.match(this._mesh.keys, this.json.keys);
 }
 ...
 TLink.prototype.createExchange = function Tlink_createExchange(){
   ...
   if (!this.x && this.csid)
     log.warn('failed to create exchange', this.json)
   else {
