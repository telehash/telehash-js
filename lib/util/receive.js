...
var route_cache = new route_cacher()
var dupe = route_cache.insert;
...
function receive_channel(mesh, packet, pipe){
  ...
  if(!link)
  {
    ...
    log.debug('mesh.index', Object.keys(mesh.index))
    return;
  }
...
function receive_handshake(mesh, packet, pipe){
  ...
  mesh.self.decrypt(packet)
           .then(function(inne){
             inner = inne;
             log.debug('inner',inner,inner.json,inner.body)
             ...
             return handshake_collect(mesh, token.toString('hex'), inner, pipe, packet);
           }).then(function(lin){
             link = lin;
             if(!link || !link.x)
               throw new Error("No link/exchange"); // can't respond w/o an exchange

             log.debug("have link for incoming handshake", link._mesh.hashname.substr)
             atOld = link.x.at();
