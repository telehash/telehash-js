...
function receive_handshake(mesh, packet, pipe){
  ...
  mesh.self.decrypt(packet)
           .then(function(inner){
             ...
             link = handshake_collect(mesh, token.toString('hex'), inner, pipe, packet);

             if(!link || !link.x)
               throw new Error("No link/exchange"); // can't respond w/o an exchange

             log.debug("have link for incoming handshake", link.label)
             atOld = link.x.at();
