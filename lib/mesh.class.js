...
function Mesh (args, cbMesh, telehash){
  ...
  this.hn = hashname.fromKeys(args.id.keys);
...
Mesh.prototype.jsonize = function Mesh_jsonize(hn, args){
  ...
  if(!json)
    json = mesh.json_store[hn] = {hashname:hn,paths:[],keys:{}};
...
{
  var json = this.json_store[mesh.hashname];
