...
function receive_channel(mesh, packet, pipe){
  ...
  if(chan )
  {
    ...
    log.debug('sequence 1 for existing channel')
    //link.x.channels[inner.json.c] = null;
  }
...
function receive_handshake(mesh, packet, pipe){
  ...
  if(!link || !link.x)
    return log.debug("can't respond without an exchance"); // can't respond w/o an exchange
  ...
  if(!sync) {
    log.debug('sending handshake back to sync')
    link.x.sending(link.handshake(),pipe);
  }
  // new outgoing sync
  if(atNew > atOld)
  {
