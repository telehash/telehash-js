...
exports.mesh = function(args, cbMesh)
{
  ...
  mesh.linked = function(opts)
  {
    var ret = [];
    Object.keys(mesh.json_store).forEach(function(hn){
      // our own json is always dynamic
      if(hn == mesh.hashname) ret.push(mesh.json());
      else ret.push(mesh.json_store[hn]);
    });
  ...
  mesh.link = function(args, cbLink)
  {
    ...
    if(!link)
    {
      link = mesh.links[args.hashname] = {hashname:args.hashname, isLink:true};
