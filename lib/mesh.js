...
// wrap raw siphash for our usage
exports.sip = {hash:function(key, value){
  if(!key || !value) return false;
  if(hashname.isHashname(key))
  {
    var binkey = base32.decode(key).slice(0,16);
    key = [];
    key[0] = binkey.readUInt32BE(0);
    key[1] = binkey.readUInt32BE(4);
    key[2] = binkey.readUInt32BE(8);
    key[3] = binkey.readUInt32BE(12);
  }
...
exports.mesh = function(args, cbMesh)
{
  ...
  {
    if(typeof uri != 'object') uri = {protocol:uri};
    if(!uri.protocol) uri.protocol = 'link';
    uri.address = mesh.public.ipv4;
    var json = mesh.json();
    uri.keys = json.keys;
  ...
  mesh.handshake = function(id, handshake, pipe, message)
  {
    ...
    if(typeof handshake.json.type != 'string') handshake.json.type = 'key';
